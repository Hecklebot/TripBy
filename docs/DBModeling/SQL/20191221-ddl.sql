...
-- 여행일차
DROP TABLE IF EXISTS `trip_day` RESTRICT;
...
CREATE TABLE `trip_detail_plans` (
	...
	`trip_time`           TIME         NOT NULL COMMENT '시간', -- 시간
...
CREATE TABLE `trip_eval` (
	...
	`scenery`       INT     NULL     COMMENT '경치' -- 경치
)
COMMENT '여행지평가';
...
CREATE TABLE `trip_notes_pic` (
	...
	`trip_day_id` INTEGER     NOT NULL COMMENT '여행일차번호', -- 여행일차번호
	`file_name`   VARCHAR(50) NOT NULL COMMENT '파일이름' -- 파일이름
)
COMMENT '여행기사진';
...
CREATE TABLE `member_reports` (
	`repo_id`   INTEGER NOT NULL COMMENT '신고자번호', -- 신고자번호
	`repoed_id` INTEGER NOT NULL COMMENT '피신고자번호', -- 피신고자번호
...
CREATE TABLE `trip_day` (
	`trip_day_id` INTEGER NOT NULL COMMENT '여행일차번호', -- 여행일차번호
	`note_id`     INTEGER NOT NULL COMMENT '여행기번호', -- 여행기번호
	`day`         DATE    NOT NULL COMMENT '일차' -- 일차
)
COMMENT '여행일차';

-- 여행일차
ALTER TABLE `trip_day`
	ADD CONSTRAINT `PK_trip_day` -- 여행일차 기본키
		PRIMARY KEY (
			`trip_day_id` -- 여행일차번호
		);

ALTER TABLE `trip_day`
	MODIFY COLUMN `trip_day_id` INTEGER NOT NULL AUTO_INCREMENT COMMENT '여행일차번호';
...
-- 여행기사진
ALTER TABLE `trip_notes_pic`
	ADD CONSTRAINT `FK_trip_day_TO_trip_notes_pic` -- 여행일차 -> 여행기사진
		FOREIGN KEY (
			`trip_day_id` -- 여행일차번호
		)
		REFERENCES `trip_day` ( -- 여행일차
			`trip_day_id` -- 여행일차번호
		);
...
-- 여행일차
ALTER TABLE `trip_day`
	ADD CONSTRAINT `FK_trip_notes_TO_trip_day` -- 여행기 -> 여행일차
		FOREIGN KEY (
			`note_id` -- 여행기번호
		)
		REFERENCES `trip_notes` ( -- 여행기
